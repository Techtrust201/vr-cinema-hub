import { Trash2, Film, ChevronDown } from "lucide-react";
...
export default function VideoRow({ video, onRemove, onUpdate }: VideoRowProps) {
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-background/40 border border-border/40 hover:border-border hover:bg-muted/30 transition-all duration-200 group">
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        {/* Format badge — clickable if onUpdate provided */}
        {onUpdate ? (
          <DropdownMenu
            trigger={
        ...
        {/* Stereo badge — clickable if onUpdate provided */}
        {onUpdate ? (
          <DropdownMenu
            trigger={
